...
function activate(context) {
    ...
    const runtimePath = config.get('runtimePath') || 'mire';
...
function deactivate() { }
function registerCommands(context, runtimePath) {
...
function getSmartCompletions(textBefore, textAfter, document, position) {
    const items = [];
...
function getTypeCompletions(word) {
...
function getIntrinsicCompletions(word) {
    const intrinsics = [
...
function createCompletion(label, detail, insertText, kind) {
        ...
        provideDocumentSymbols(document, token) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (const pattern of patterns) {
                    ...
                    if (match) {
                        const symbol = new vscode.DocumentSymbol(match[1], pattern.detail, pattern.kind, new vscode.Range(i, 0, i, line.length), new vscode.Range(i, 0, i, line.length));
                        symbols.push(symbol);
                    }
                }
            }
