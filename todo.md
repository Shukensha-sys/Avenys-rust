...
- ✅ Completado: segunda fase de modularización:
  - parser imports extraídos a `src/parser/imports.rs`
  - parser type/generic helpers extraídos a `src/parser/types.rs`
  - type checker builtins/std-import helpers extraídos a `src/compiler/typeck/typeck_builtins.rs`
...
1. Modularización del parser (`src/parser/mod.rs`) en submódulos restantes (`statements`, `expressions`, `patterns`).
2. Modularización de `typeck` (`src/compiler/typeck.rs`) en capas (`signatures`, `statements`, `expressions`, `types`, `errors`), partiendo de fases ya extraídas (`typeck_returns`, `typeck_builtins`).
