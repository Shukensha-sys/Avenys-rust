...
- Fase 1: hash streaming con `serde_json::to_writer` sobre `FxHasher` para eliminar buffer intermedio.
- Fase 2 (v2.4.x): migración completa a hash estructural directo del AST
  (`Statement`/`Expression`/`DataType`/`MireValue`) sin serialización JSON.
- Regresiones añadidas:
  - determinismo para el mismo statement
  - sensibilidad al cambio de statement
  - equivalencia funcional del invalidation report optimizado vs naive
...
- Regresiones añadidas para verificar no-fuga de contexto entre runs y adjunte correcto con `origins`.
