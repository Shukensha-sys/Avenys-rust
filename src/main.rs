...
use mire::CacheOverrides;
use mire::MireCacheConfig;
use mire::MireError;
...
fn create_project_in_dir(project_dir: &Path, manifest_dir: &Path) -> Result<i32, MireError> {
    ...
    let manifest = MireManifest {
        ...
        cache: Some(MireCacheConfig {
            max_units: Some(256),
            analysis_cache: Some(true),
...
fn parse_debug_options(cwd: &Path, args: &[String]) -> Result<DebugOptions, MireError> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--cache-compression" => cache.compression = Some(true),
            "--no-cache-compression" => cache.compression = Some(false),
            "--cache-max-units" => {
                index += 1;
