    ...
    pub(super) fn check_expression(&mut self, expression: &mut Expression) -> Result<DataType> {
        ...
        match expression {
            ...
            Expression::Call {
                ...
                Err(type_error(self.current_line, self.current_column, format!("Unknown function '{}'", name)))
            }
            Expression::List {
                elements,
                element_type,
            ...
            Expression::Index {
                ...
                let resolved = match target_type {
                    ...
                    other => {
                        return Err(type_error(self.current_line, self.current_column, format!("Type {:?} is not indexable", other)));
                    }
                };
            ...
            Expression::EnumVariantPath {
                ...
                if !self.enum_variants.contains_key(&full_name) {
                    return Err(type_error(self.current_line, self.current_column, format!("Unknown enum variant '{}'", full_name)));
                }
                *data_type = DataType::EnumNamed(enum_name.clone());
            ...
            Expression::EnumVariant {
                ...
                let variant_sig =
                    self.enum_variants.get(&full_name).cloned().ok_or_else(|| {
                        type_error(self.current_line, self.current_column, format!("Unknown enum variant '{}'", typed_name))
                    })?;
